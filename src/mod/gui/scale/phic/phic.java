...
import mod.model.controller.PHICC;
import mod.others.myFunction;

public class phic extends javax.swing.JInternalFrame {
    ...
    public static phic_add_range frmNew;
    public static phic_edit_range frmEdit;
    PHIC phic = new PHIC();
    PHICC phicc = new PHICC();
    ArrayList<PHIC> PHICTemp = new ArrayList<>();
    ...
    void setTableModels() {
        tbl.setModel(new PHICC().getPHICTableModel(tbl));
    }

    public void ShowFrmNew() {
        frmNew = new phic_add_range(this, true);
        frmNew.setVisible(true);
    }

    public void ShowFrmEdit() {
        frmEdit = new phic_edit_range(this, true);
    ...
    private void initComponents() {
        ...
        setMaximizable(true);
        setResizable(true);
        setTitle("PHIC Contribultion Scale Table");
        setFrameIcon(new javax.swing.ImageIcon(getClass().getResource("/images/philhealth.png"))); // NOI18N
        ...
        jLabel1.setIcon(new javax.swing.ImageIcon(getClass().getResource("/images/philhealthb.png"))); // NOI18N
        ...
        try {
            ...
            if (tbl.isRowSelected(row) != true) {
                ...
                if (i == 0) {
                    ...
                    phic.setPhicId(Short.parseShort(id));

                    if (phicc.deletePHIC(phic) > 0) {
                        setTableModels();
                        //this.dispose();
                    } else {
    ...
    void move_row(int move) {//UP OR DOWN LOCATION
        ...
        prevcol[ip++] = myFunction.amountFormat2(phicc.GetRangeStart(Integer.parseInt(pid)));
        prevcol[ip++] = myFunction.amountFormat2(phicc.GetRangeEnd(Integer.parseInt(pid)));
        prevcol[ip++] = myFunction.amountFormat2(phicc.GetSalaryBase(Integer.parseInt(pid)));
        prevcol[ip++] = myFunction.amountFormat2(phicc.GetEE(Integer.parseInt(pid)));
        prevcol[ip++] = myFunction.amountFormat2(phicc.GetER(Integer.parseInt(pid)));
        prevcol[ip++] = myFunction.amountFormat2(phicc.GetTotal(Integer.parseInt(pid)));
        ...
        selcol[is++] = myFunction.amountFormat2(phicc.GetRangeStart(Integer.parseInt(sid)));
        selcol[is++] = myFunction.amountFormat2(phicc.GetRangeEnd(Integer.parseInt(sid)));
        selcol[is++] = myFunction.amountFormat2(phicc.GetSalaryBase(Integer.parseInt(sid)));
        selcol[is++] = myFunction.amountFormat2(phicc.GetEE(Integer.parseInt(sid)));
        selcol[is++] = myFunction.amountFormat2(phicc.GetER(Integer.parseInt(sid)));
        selcol[is++] = myFunction.amountFormat2(phicc.GetTotal(Integer.parseInt(sid)));
    ...
    public void setPHICTempDatas() {
        ...
        while (cr < rc + 1) {
            try {
                ...
                //SET DATA OF ROW
                start = phicc.GetRangeStart(Integer.parseInt(sid));
                end = phicc.GetRangeEnd(Integer.parseInt(sid));
                sbase = phicc.GetSalaryBase(Integer.parseInt(sid));
                ee = phicc.GetEE(Integer.parseInt(sid));
                er = phicc.GetER(Integer.parseInt(sid));
                ec = phicc.GetTotal(Integer.parseInt(sid));
    ...
    void rePersistDatas() {
        for (PHIC phic : PHICTemp) {
            phic.setRangeStart(BigDecimal.valueOf(Double.parseDouble(phic.getRangeStart().toString())));
            phic.setRangeEnd(BigDecimal.valueOf(Double.parseDouble(phic.getRangeEnd().toString())));
            phic.setSalaryBase(BigDecimal.valueOf(Double.parseDouble(phic.getSalaryBase().toString())));
            phic.setEe(BigDecimal.valueOf(Double.parseDouble(phic.getEe().toString())));
            phic.setEr(BigDecimal.valueOf(Double.parseDouble(phic.getEe().toString())));
            phic.setTotal(BigDecimal.valueOf(Double.parseDouble(phic.getEe().toString())));

            if (phicc.addPHIC(phic) > 0) {
            } else {
    ...
    void deleteAllPHICDatas() {
        if (phicc.deleteAll(phic) > 0) {
        } else {
            JOptionPane.showMessageDialog(null, "Failed to save");
        }
    }
