...
public class phic_edit_range extends javax.swing.JDialog {
    
    public static phic frmParent;
    static Statement stmt;
    public static String phicid;
    PHIC phic = new PHIC();
    PHICC phicc = new PHICC();
    
    public phic_edit_range(phic parent, boolean modal) {
    ...
    public void populateFields() {
        ...
        String createString;
        createString = "SELECT range_start, range_end, salary_base, ee, er, total "
                + "FROM phic WHERE phic_id=" + phicid;
        try {
    ...
    private void cmdsaveActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmdsaveActionPerformed
        ...
        if (start.isEmpty() == true || end.isEmpty() == true || base.isEmpty() == true || ee.isEmpty() == true || er.isEmpty() == true || total.isEmpty() == true) {
            ...
            phic.setRangeStart(bstart);
            phic.setRangeEnd(bend);
            phic.setSalaryBase(bbase);
            phic.setTotal(btotal);
            phic.setEe(bee);
            phic.setEr(ber);
            phic.setPhicId(Short.parseShort(phicid));
            
            
            if (phicc.editPHIC(phic) > 0) {
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                phic_edit_range dialog = new phic_edit_range(frmParent, true);
