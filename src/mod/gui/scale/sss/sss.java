...
import mod.model.controller.SSSC;
import mod.others.myFunction;

public class sss extends javax.swing.JInternalFrame {
    ...
    public static sss_add_range frmNew;
    public static sss_edit_range frmEdit;
    SSS sss = new SSS();
    SSSC sssc = new SSSC();
    ArrayList<SSS> SSSTemp = new ArrayList<>();
    ...
    void setTableModels() {
        tbl.setModel(new SSSC().getSSSTableModel(tbl));
    }

    public void ShowFrmNew() {
        frmNew = new sss_add_range(this, true);
        frmNew.setVisible(true);
    }

    public void ShowFrmEdit() {
        frmEdit = new sss_edit_range(this, true);
    ...
    private void initComponents() {
        ...
        setMaximizable(true);
        setResizable(true);
        setTitle("SSS Contribultion Scale Table");
        setFrameIcon(new javax.swing.ImageIcon(getClass().getResource("/images/sss.png"))); // NOI18N
        ...
        tbl.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "id", "Range No.", "Range Start", "Range End", "Salary Base", "EE", "ER", "EC"
            }
        ) {
        ...
        jLabel1.setIcon(new javax.swing.ImageIcon(getClass().getResource("/images/sssb.png"))); // NOI18N
        ...
        try {
            ...
            if (tbl.isRowSelected(row) != true) {
                ...
                if (i == 0) {
                    ...
                    sss.setSssId(Short.parseShort(id));

                    if (sssc.deleteSSS(sss) > 0) {
                        setTableModels();
                        //this.dispose();
                    } else {
    ...
    void move_row(int move) {//UP OR DOWN LOCATION
        ...
        prevcol[ip++] = myFunction.amountFormat2(sssc.GetRangeStart(Integer.parseInt(pid)));
        prevcol[ip++] = myFunction.amountFormat2(sssc.GetRangeEnd(Integer.parseInt(pid)));
        prevcol[ip++] = myFunction.amountFormat2(sssc.GetSalaryBase(Integer.parseInt(pid)));
        prevcol[ip++] = myFunction.amountFormat2(sssc.GetEE(Integer.parseInt(pid)));
        prevcol[ip++] = myFunction.amountFormat2(sssc.GetER(Integer.parseInt(pid)));
        prevcol[ip++] = myFunction.amountFormat2(sssc.GetEC(Integer.parseInt(pid)));
        ...
        selcol[is++] = myFunction.amountFormat2(sssc.GetRangeStart(Integer.parseInt(sid)));
        selcol[is++] = myFunction.amountFormat2(sssc.GetRangeEnd(Integer.parseInt(sid)));
        selcol[is++] = myFunction.amountFormat2(sssc.GetSalaryBase(Integer.parseInt(sid)));
        selcol[is++] = myFunction.amountFormat2(sssc.GetEE(Integer.parseInt(sid)));
        selcol[is++] = myFunction.amountFormat2(sssc.GetER(Integer.parseInt(sid)));
        selcol[is++] = myFunction.amountFormat2(sssc.GetEC(Integer.parseInt(sid)));
    ...
    public void setSSSTempDatas() {
        ...
        while (cr < rc + 1) {
            try {
                ...
                //SET DATA OF ROW
                start = sssc.GetRangeStart(Integer.parseInt(sid));
                end = sssc.GetRangeEnd(Integer.parseInt(sid));
                sbase = sssc.GetSalaryBase(Integer.parseInt(sid));
                ee = sssc.GetEE(Integer.parseInt(sid));
                er = sssc.GetER(Integer.parseInt(sid));
                ec = sssc.GetEC(Integer.parseInt(sid));
                ...
                list.setEc(BigDecimal.valueOf(Double.parseDouble(ec)));
                SSSTemp.add(list);
            } catch (Exception e) {
                e.getStackTrace();
            }
    ...
    void rePersistDatas() {
        for (SSS sss : SSSTemp) {
            sss.setRangeStart(BigDecimal.valueOf(Double.parseDouble(sss.getRangeStart().toString())));
            sss.setRangeEnd(BigDecimal.valueOf(Double.parseDouble(sss.getRangeEnd().toString())));
            sss.setSalaryBase(BigDecimal.valueOf(Double.parseDouble(sss.getSalaryBase().toString())));
            sss.setEe(BigDecimal.valueOf(Double.parseDouble(sss.getEe().toString())));
            sss.setEr(BigDecimal.valueOf(Double.parseDouble(sss.getEe().toString())));
            sss.setEc(BigDecimal.valueOf(Double.parseDouble(sss.getEe().toString())));

            if (sssc.addSSS(sss) > 0) {
            } else {
    ...
    void deleteAllSSSDatas() {
        if (sssc.deleteAll(sss) > 0) {
        } else {
            JOptionPane.showMessageDialog(null, "Failed to save");
        }
    }
